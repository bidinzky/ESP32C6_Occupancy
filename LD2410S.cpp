...
void LD2410S::setup(Stream &serial) {
    this->serial = &serial;
...
void LD2410S::loop() {
    if (!this->cmd_active) {
        ...
        while (serial->available()) {
            PackageType type = this->read_line(serial->read(), buffer, pos++);
...
void LD2410S::send_command(CmdFrameT frame)
{
    ...
    while (retry)
    {
        ...
        while (!reply) {
            ...
            size_t last_pos = 0;
            while (serial->available()) {
                PackageType type = this->read_line(serial->read(), ack_buffer, last_pos++);
...
void LD2410S::process_config_read_ack(uint8_t* data) {
    ...
    this->max_distance_number = max_dist;
    this->min_distance_number = min_dist;
    this->no_delay_number= delay;
    this->status_reporting_freq_number = status_resp_freq / 10;
    this->distance_reporting_freq_number = dist_resp_freq / 10;
#ifdef USE_SELECT
    this->response_speed_select->publish_state(resp_speed == 5 ? RESPONSE_SPEED_NORMAL : RESPONSE_SPEED_FAST);
#endif
    memcpy(&this->new_config, &this->current_config, sizeof(this->current_config));
    char s[255];
    sprintf(s, "Read config replay: max_dist=%d, min_dist=%d, delay=%d, status_resp_freq=%d, dist_resp_freq=%d, resp_speed=%d", max_dist, min_dist, delay, status_resp_freq, dist_resp_freq, resp_speed);
    Serial.println(s);
}
...
bool LD2410S::process_cmd_ack_package(uint8_t* buffer, int len) {
    ...
    case START_CONFIG_MODE_REPLAY:
        Serial.println("[sensor]: Config mode enabled");
        //ESP_LOGD(TAG, "Config mode enabled");
        break;
    case END_CONFIG_MODE_REPLAY:
        Serial.println("[sensor]: Config mode disabled");
    ...
    case WRITE_PARAMS_REPLAY:
     Serial.println("[sensor]: Write config replay processed");
